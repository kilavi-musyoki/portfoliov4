...
const DebugOverlay = ({ visible, fps, isDark = true }) => {
    ...
    const headerBorder = isDark ? 'rgba(75,216,160,0.38)' : 'rgba(80,177,206,0.38)';
    const activeLabel = isDark ? '#FF5A3C' : '#e05c3a';
    const operatorColor = isDark ? '#ced0ce' : '#2A2A3A';
    const clearanceC = isDark ? '#D4A843' : '#c4973a';
    ...
    const sectionHeader = isDark ? 'rgba(75,216,160,0.52)' : 'rgba(80,177,206,0.62)';
    const sectionName = isDark ? '#ced0ce' : '#50b1ce';
    const sectionBounds = isDark ? 'rgba(75,216,160,0.42)' : 'rgba(80,177,206,0.48)';
    const sectionVar = isDark ? '#79bfc9' : '#2A2A3A';
    const dismissColor = isDark ? 'rgba(75,216,160,0.32)' : 'rgba(80,177,206,0.42)';
    const outlineSection = isDark ? 'rgba(75,216,160,0.14)' : 'rgba(80,177,206,0.18)';
    const outlineDebug = isDark ? 'rgba(77,255,255,0.28)' : 'rgba(80,177,206,0.42)';
    ...
    return (
        ...
        >
            ...
            <div style={{ borderBottom: `1px solid ${headerBorder}`, paddingBottom: '8px', marginBottom: '8px' }}>
                ...
                {'  '}
                <span style={{ color: clearanceC }}>CLEARANCE: LEVEL 4</span>
            </div>
